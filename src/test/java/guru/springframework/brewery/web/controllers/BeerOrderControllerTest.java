...
//		given(beerOrderService.placeOrder(beerOrderDto.getCustomerId(), beerOrderDto)).willReturn(beerOrderDto);
//
//		mockMvc.perform(post("/api/v1/customers/" + beerOrderDto.getCustomerId() + "/orders", beerOrderDto))
//				.andExpect(status().isCreated()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8));
//	}

	@Test
	void getOrder() throws Exception {
		given(beerOrderService.getOrderById(beerOrder.getCustomerId(), beerOrder.getId()))
				.willReturn(beerOrder);

		mockMvc.perform(get("/api/v1/customers/" + beerOrder.getCustomerId() + "/orders/" + beerOrder.getId()))
